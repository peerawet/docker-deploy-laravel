...
@section('content')
    ...
    <div id="destination">
        ...
        <div class="box p-5">
            ...
            <div class="mb-5">
                <div class="grid grid-cols-12 gap-2">
                    <form id="search-form" class="form-inline col-span-4"
                        action="{{ route('masterFiles.index.destination-port') }}" method="GET">
                    ...
                    <thead class="table-dark">
                        <tr class="text-center">
                            <th class="whitespace-nowrap">
                                <a
                                    href="{{ route('masterFiles.index.destination-port', [
                                        'sort_field' => 'id',
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="modal-title">Add New Port</h5>
                            <button type="button" class="btn-close" data-tw-dismiss="modal"
                                aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <!-- Add New Port form -->
                            <form id="add-port-form" method="POST" action="{{ route('add.destination-port') }}">
                                @csrf
            ...
            <!-- END: Add New Port Modal -->


            <!-- Delete Confirmation Modal for Port -->
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body p-0">
                            <div class="p-5 text-center">
                                ...
                                <!-- Updated placeholder id -->
                            </div>
                            <div class="px-5 pb-8 text-center">
                                <form id="delete-port-form" method="POST"
                                    action="{{ route('delete.destination-port', ['id' => '__PORT_ID__']) }}">
                                    <!-- Updated form action -->
                                    @csrf
            ...
            <!-- END: Delete Confirmation Modal for Port -->
            ...
            <div id="edit-port-modal" class="modal" tabindex="-1" aria-labelledby="modal-title" role="dialog"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="modal-title">Edit Port</h5>
                        ...
                        <div class="modal-body">
                            ...
                            <form id="edit-port-form" method="POST"
                                action="{{ route('edit.destination-port', ['id' => '__PORT_ID__']) }}">
                                <!-- Updated form action -->
                                @csrf
                                <input type="hidden" name="id" id="edit-port-id" value="">
                                <!-- Updated input name -->
                                <div class="form-group">
                                    <label for="edit-port-name">Port Name</label> <!-- Updated label -->
                                    <input type="text" class="form-control" id="edit-port-name" name="name"
                                        value="">
                                </div>
                                <div class="form-group mt-3">
                                    <label for="edit-port-note">Note</label> <!-- Updated label -->
    ...
    <script>
        jQuery(document).ready(function() {
            ...
            jQuery('.delete-port-btn').on('click', function() {
                ...
                var formAction = "{{ route('delete.destination-port', ['id' => '__PORT_ID__']) }}";
                ...
                $('#port-name-placeholder').text('Do you want to delete Destination Port ' +
                    portName + '?');
            });
        });

        jQuery(document).ready(function() {
            // Edit Destination Port modal
            jQuery('.edit-port-btn').on('click', function() {
                // Get the Port ID and name from the clicked button's data attributes
                var portIdToEdit = $(this).data('port-id');
                var portName = $(this).data('port-name');
                var portNote = $(this).data('port-note'); // Assuming you have a data attribute for note

                // Set the form action dynamically
                var formAction = "{{ route('edit.destination-port', ['id' => '__PORT_ID__']) }}";
                ...
                $('#edit-port-note').val(portNote); // Populate the note field
            });
