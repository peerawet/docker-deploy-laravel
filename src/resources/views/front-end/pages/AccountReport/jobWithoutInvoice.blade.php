...
@section('title', 'Job ที่ยังไม่ออก Invoice')
@section('content')
    ...
    <script>
        ...
        document.getElementById('customTable').addEventListener('click', function() {
            ...
            var url = "{{ route('account.jobs-without-invoice', ['start' => ':start', 'end' => ':end']) }}";
        ...
        <div id="content-to-be-converted">
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 20px;" class="font">
                <div style="flex: 1; text-align: center;">
                    <strong class="font" style="font-size: 20px;">Draft/ Job ที่ไม่ออก Invoice เรียงตาม Job No.</strong>
                </div>
                <div>
                    <strong class="font">Job Date: {{ $start }} - {{ $end }}</strong>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($jobs as $job)
                            <tr class="main-row" data-job-no="{{ $job->job_no }}">
                                <td>{{ optional($job->draft)->draft_no }}</td>
                                <td>{{ optional($job->draft)->status }}</td>
                                <td>{{ $job->job_no }}</td>
                                <td>{{ $job->job_date ? date('d/m/Y', strtotime($job->job_date)) : '' }}</td>
                                ...
                                <td>{{ number_format($job->sell_total_vat, 2) }}</td>
                                <td>{{ number_format($job->sell_grand_total, 2) }}</td>
                            </tr>
                            @php
                                $total_vat += $job->sell_total_vat;
                                $total_grand_total += $job->sell_grand_total;
                                $item_count++;
                            @endphp
                        @endforeach
                    </tbody>
