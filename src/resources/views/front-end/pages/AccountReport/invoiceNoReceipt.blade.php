...
@section('title', 'Invoice เก็บเงินแล้วแต่ไม่ออกใบเสร็จ')
...
@section('content')
    ...
    <script>
        ...
        document.getElementById('customTable').addEventListener('click', function() {
            ...
            var url = "{{ route('account.invoice-no-receipt', ['start' => ':start', 'end' => ':end']) }}";
        ...
        <div id="content-to-be-converted">
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 20px;" class="font">
                <div style="flex: 1; text-align: center;">
                    <strong class="font" style="font-size: 20px;">Invoice เก็บเงินแล้วไม่ออกใบเสร็จ</strong>
                </div>
                <div>
            ...
            <div class="overflow-x-auto">
                <table class="table table-bordered table-striped table-hover" style="text-align: center;" class="font">
                    <thead class="table-dark">
                        <tr>
                            <th class="whitespace-nowrap">Draft No</th>
                            <th class="whitespace-nowrap">Status</th>
                            <th class="whitespace-nowrap">Job No.</th>
                            <th class="whitespace-nowrap">Job Date</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($invoices as $invoice)
                            <tr class="main-row" data-invoice-no="{{ $invoice->invoice_no }}">
                                <td>{{ optional(optional($invoice->job)->draft)->draft_no }}</td>
                                <td>{{ optional(optional($invoice->job)->draft)->status }}</td>
                                <td>{{ optional($invoice->job)->job_no }}</td>
                                <td>{{ optional($invoice->job)->job_date ? date('d/m/Y', strtotime(optional($invoice->job)->job_date)) : '' }}
                                </td>
